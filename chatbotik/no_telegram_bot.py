...
def handle_webapp_order(user_id, web_app_data):
    ...
    try:
        ...
        if order_data.get('action') == 'order' or order_data.get('type') == 'order':
            ...
            # Уведомление клиента - профессиональный стиль
            client_message = (
                ...
                f"<b>ДОСТАВКА:</b> Уточним при подтверждении\n\n"
                f"Менеджер свяжется с вами в течение 15 минут для уточнения деталей.\n\n"
                f"📱 /myorders — история заказов"
            )
            send_message(user_id, client_message)
            ...
            # Получаем номер телефона из переменных окружения
            admin_phone = os.getenv('ADMIN_PHONE', '+7 (999) 123-45-67')
            
            for admin_id in admins_from_db:
                try:
                    # СООБЩЕНИЕ 1: Информация о заказе с нумерацией
                    admin_message = (
                    ...
                    send_message(admin_id, sbp_template)
                    
                    sent_count += 1
                    logger.info(f"✅ Заказ и шаблон СБП отправлены администратору {admin_id}")
                except Exception as e:
