        ...
        if order_data.get('action') == 'order' or order_data.get('type') == 'order':
            ...
            for admin_id in admins_from_db:
                try:
                    ...
                    # Кнопка для перехода в чат с клиентом
                    keyboard = {
                        'inline_keyboard': [[
                            {
                    ...
                    # СООБЩЕНИЕ 2: Готовый шаблон для копирования
                    template_message = (
                    ...
                    send_message(admin_id, template_message)
                    
                    sent_count += 1
                    logger.info(f"✅ Заказ, кнопка и шаблон отправлены администратору {admin_id}")
                except Exception as e:
