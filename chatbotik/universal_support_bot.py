#!/usr/bin/env python3
"""
🤖 УНИВЕРСАЛЬНЫЙ БОТ ПОДДЕРЖКИ
==============================
Работает с любой версией python-telegram-bot
"""
...
# Глобальное хранилище для рейтингов (упрощенно)
pending_ratings = {}
...
def start_command(bot, update):
    ...
    try:
        ...
        if is_admin(user_id):
            # Меню для администратора - простой текст
            message = (
            ...
            # Меню для клиента - простой текст
            message = (
                "👋 *Добро пожаловать!*\n\n"
...
def handle_text(bot, update):
    ...
    try:
        ...
        if user_id in pending_ratings:
            save_review(bot, update, message_text)
            return
...
def save_review(bot, update, review_text):
    """Сохранение отзыва"""
    try:
        user = update.effective_user
        user_id = user.id
        rating = pending_ratings.get(user_id, 0)
        ...
        logger.error(f"Ошибка в save_review: {e}")
...
def main():
    ...
    print("🤖 ЗАПУСК УНИВЕРСАЛЬНОГО БОТА ПОДДЕРЖКИ")
    print("=" * 50)
