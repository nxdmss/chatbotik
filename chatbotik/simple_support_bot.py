#!/usr/bin/env python3
"""
🤖 ПРОСТОЙ БОТ ПОДДЕРЖКИ КЛИЕНТОВ
=================================
...
def handle_support_text(update: Update, context: CallbackContext):
...
def handle_customer_message(update: Update, context: CallbackContext):
    ...
    if message_text == "📞 Связь с администратором":
        ...
        if 'pending_rating' in context.user_data:
            save_review(update, context, message_text)
        else:
            # Сообщение для администратора
            forward_to_admin(update, context)

def save_review(update: Update, context: CallbackContext, review_text: str):
    """Сохранение отзыва"""
    user = update.effective_user
    user_id = user.id
    ...
    if rating == 0:
        update.message.reply_text("❌ Ошибка: рейтинг не найден. Попробуйте снова.")
        return
...
def handle_admin_message(update: Update, context: CallbackContext):
    ...
    if message_text == "📋 Список клиентов":
        show_customers_list(update, context)
    elif message_text == "💬 Сообщения поддержки":
        show_support_messages(update, context)
    elif message_text == "⭐ Отзывы":
        show_admin_reviews(update, context)
...
def show_customers_list(update: Update, context: CallbackContext):
...
def show_support_messages(update: Update, context: CallbackContext):
...
def show_admin_reviews(update: Update, context: CallbackContext):
...
def callback_query_handler(update: Update, context: CallbackContext):
    ...
    if data == "leave_review":
        leave_review(update, context)
    elif data == "view_reviews":
        view_reviews(update, context)
    elif data.startswith("rating_"):
        rating = int(data.split("_")[1])
        process_review_rating(update, context, rating)
...
def process_review_rating(update: Update, context: CallbackContext, rating: int):
...
def create_order_command(update: Update, context: CallbackContext):
...
def update_order_command(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(f"❌ Ошибка обновления заказа: {e}")
...
def main():
    ...
    dispatcher.add_handler(CommandHandler("update_order", update_order_command))
