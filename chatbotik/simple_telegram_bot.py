        ...
        if self.path == '/api/orders':
            ...
            try:
                ...
                if BOT_TOKEN:
                    ...
                    # URL для кнопки (открывает чат с клиентом и вставляет шаблон)
                    import urllib.parse
                    encoded_template = urllib.parse.quote(client_template)
                    chat_url = f"tg://user?id={user_id}&text={encoded_template}"
                    ...
                    try:
                        ...
                        # Отправляем информацию о заказе с кнопкой
                        telegram_data = {
                            'chat_id': ADMIN_ID,
                            'text': admin_msg,
                            'parse_mode': 'HTML',
                        ...
                        if resp.json().get('ok'):
                            print(f"✅ [1/2] Заказ отправлен администратору {ADMIN_ID}")
                            
                            # Отправляем шаблон отдельным сообщением (для копирования)
                            template_msg = (
                                f"📋 <b>ГОТОВЫЙ ТЕКСТ ДЛЯ КЛИЕНТА:</b>\n"
                                f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                                f"<code>{client_template}</code>\n\n"
                                f"<i>Скопируйте текст выше и отправьте клиенту</i>"
                            )
                            ...
                            if resp2.json().get('ok'):
                                print(f"✅ [2/2] Шаблон отправлен администратору {ADMIN_ID}")
                                print(f"✅✅✅ ЗАКАЗ ПОЛНОСТЬЮ ОТПРАВЛЕН!")
                            else:
                                print(f"⚠️ Ошибка отправки шаблона: {resp2.text}")
                        else:
