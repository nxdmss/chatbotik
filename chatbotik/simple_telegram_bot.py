            ...
            try:
                ...
                if BOT_TOKEN:
                    ...
                    admin_msg = (
                        ...
                        f"3️⃣ Отправить реквизиты для оплаты (шаблон ниже ⬇️)\n\n"
                        f"⏱ <b>Обработать в течение 15 минут</b>"
                    )
                    
                    # СООБЩЕНИЕ 2: Шаблон СБП для отправки клиенту
                    sbp_template = (
                        f"📋 <b>ШАБЛОН ДЛЯ ОТПРАВКИ КЛИЕНТУ:</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                        f"Здравствуйте! Ваш заказ #{order_number} подтвержден.\n\n"
                        f"<b>К оплате: {total} ₽</b>\n\n"
                        f"<b>ОПЛАТА СБП (без комиссии):</b>\n"
                        f"📱 Номер: <code>{admin_phone}</code>\n"
                        f"👤 Получатель: Администратор магазина\n\n"
                        f"<b>Другие способы:</b>\n"
                        f"💳 Карта Сбербанк: по запросу\n"
                        f"💵 Наличные: при получении\n\n"
                        f"После оплаты отправьте скриншот чека.\n"
                        f"Доставка в течение 1-3 дней."
                    )
                    ...
                    try:
                        ...
                        # Отправляем сообщение 1: Информация о заказе
                        telegram_data_1 = {
                            'chat_id': ADMIN_ID,
                            'text': admin_msg,
                            'parse_mode': 'HTML'
                        }
                        resp1 = requests.post(telegram_url, json=telegram_data_1, timeout=10)
                        
                        if resp1.json().get('ok'):
                            print(f"✅ [1/2] Информация о заказе отправлена администратору {ADMIN_ID}")
                            
                            # Отправляем сообщение 2: Шаблон СБП
                            telegram_data_2 = {
                                'chat_id': ADMIN_ID,
                                'text': sbp_template,
                                'parse_mode': 'HTML'
                            }
                            resp2 = requests.post(telegram_url, json=telegram_data_2, timeout=10)
                            
                            if resp2.json().get('ok'):
                                print(f"✅ [2/2] Шаблон СБП отправлен администратору {ADMIN_ID}")
                                print(f"✅✅✅ ЗАКАЗ ПОЛНОСТЬЮ ОТПРАВЛЕН АДМИНИСТРАТОРУ!")
                            else:
                                print(f"⚠️ Ошибка отправки шаблона СБП: {resp2.text}")
                        else:
                            print(f"❌ Ошибка отправки информации: {resp1.text}")
...
💰 Итого: {totals.get('total'):,} ₽"""
                    ...
                    # ШАБЛОН СБП ДЛЯ АДМИНА (для копирования и отправки клиенту)
                    sbp_phone = os.getenv('SBP_PHONE', '+79991234567')
                    template_msg = f"""📋 ШАБЛОН ДЛЯ КЛИЕНТА (скопируйте и отправьте):
...
💡 Скопируйте это сообщение и отправьте клиенту"""
                    
                    if customer.get('telegram_username'):
                        template_msg += f" @{customer.get('telegram_username')}"
                    
                    try:
                        requests.post(f'https://api.telegram.org/bot{bot_token}/sendMessage', json={'chat_id': admin_id, 'text': template_msg}, timeout=10)
                        print("✅ Шаблон СБП отправлен админу")
                    except Exception as e:
                        print(f"⚠️ Ошибка отправки шаблона: {e}")
