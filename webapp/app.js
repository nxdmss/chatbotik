...
console.log('🚀 Загружается приложение версии 6.0 - АДМИНСКИЕ ПРАВА ВКЛЮЧЕНЫ АВТОМАТИЧЕСКИ В TELEGRAM WEBAPP');
...
class MobileShopApp {
    ...
    async checkAdminStatus() {
        try {
            ...
            // ВРЕМЕННО: Принудительно даем админские права в Telegram WebApp
            if (window.Telegram && window.Telegram.WebApp) {
                console.log('🔧 ПРИНУДИТЕЛЬНО ВКЛЮЧАЕМ АДМИНСКИЕ ПРАВА ДЛЯ TELEGRAM WEBAPP');
                this.isAdmin = true;
                console.log('👑 Админские права принудительно включены для Telegram WebApp');
            }
